...
export async function getMe() {
  const res = await api.get<ApiResponse<CurrentUserResponse>>('/v1/me')
  return unwrap(res)
}
...
export async function updateMe(body: UserUpdateRequest) {
  const res = await api.put<ApiResponse<CurrentUserResponse>>('/v1/me', body)
  return unwrap(res)
}
